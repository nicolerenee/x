...
func TestNewID(t *testing.T) {
	...
	require.Equal(t, 7, idx.PrefixPartLength)
...
func TestParsers(t *testing.T) {
	cases := []struct {
		...
		{name: "valid id", id: string(idx.MustNewID("testing"))},
	...
	t.Run("Test idx.Parse", func(t *testing.T) {
		for _, tt := range cases {
			t.Run(tt.name, func(t *testing.T) {
				id, err := idx.Parse(tt.id)
				if tt.errorMsg == "" {
					assert.NoError(t, err)
					assert.NotNil(t, id)
				} else {
	...
	t.Run("Test Value", func(t *testing.T) {
		for _, tt := range cases {
			t.Run(tt.name, func(t *testing.T) {
				v, err := idx.PrefixedID(tt.id).Value()
				if tt.errorMsg == "" {
					assert.NoError(t, err)
					assert.NotNil(t, v)
				} else {
